...
const Dashboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [businessRes, salesRes, productsRes, invoicesRes] = await Promise.allSettled([
          api.get("/business"),
          api.get("/sales"),
          api.get("/products"),
          api.get("/invoices"),
        ...
        if (invoicesRes.status === "fulfilled") {
          const unpaidCount = invoicesRes.value.data.invoices.filter(
            (inv) => inv.status === "unpaid",
          ).length;
          setOutstandingInvoices(unpaidCount);
        }
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard label="Outstanding Invoices" value={outstandingInvoices} />
      </div>
      ...
      <div className="mt-8 mb-8 rounded-xl border border-blue-100 bg-blue-50 p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
          ...
          <Button onClick={() => navigate("/storefront")}>
            {business?.storefrontEnabled ? "Manage Storefront" : "Set Up Storefront"}
          </Button>
        </div>
      </div>
